...
2. **环境变量已配置**：
   ...
   - `JIMENG_MODEL`：模型名称（默认：jimeng-4.0）
...
使用即梦 4.0 引擎从文本描述生成图像。

**工具**：`text_to_image`

**参数：**
- `prompt`（必需）：期望图像的文本描述
- `width`（可选）：图像宽度，单位像素（512, 768, 1024, 1536, 2048）
- `height`（可选）：图像高度，单位像素（512, 768, 864, 1024, 2048）
- `sample_strength`（可选）：采样强度 0.0-1.0，控制创意性（默认：0.5）
- `negative_prompt`（可选）：要在生成图像中避免的元素
...
**提示：**
- 更高的 `sample_strength`（0.7-1.0）= 更有创意/抽象
- 更低的 `sample_strength`（0.3-0.5）= 更真实/字面化
- 使用描述性提示词以获得更好的结果
- 指定艺术风格、光照、氛围以增强控制
...
- `width`（可选）：输出宽度（512, 768, 1024, 1536, 2048）
- `height`（可选）：输出高度（512, 768, 864, 1024, 2048）
- `sample_strength`（可选）：合成强度 0.0-1.0（默认：0.5）
...
**参数：**
- `prompt`（必需）：视频场景的文本描述
- `width`（可选）：视频宽度（480, 720, 1280, 1920）
- `height`（可选）：视频高度（480, 720, 1080）
...
- `width`（可选）：输出宽度（480, 720, 1280, 1920）
- `height`（可选）：输出高度（480, 720, 1080）
- `resolution`（可选）：预设分辨率（"480p", "720p", "1080p"）
...
1. **提取提示词**：用户对期望内容的描述
2. **识别尺寸**：提取尺寸偏好（横向/竖向/正方形）
3. **解析质量设置**：寻找创意性/真实性指标
...
**加快生成速度：**
- 使用较低分辨率（720p 而非 1080p）
- 降低 `sample_strength` 以加快处理
- 保持提示词简洁但具有描述性

**提高质量：**
- 使用详细、具体的提示词
- 实验不同的 `sample_strength` 值
- 指定艺术风格和技术
- 包含光照和氛围描述
